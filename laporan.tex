...
\begin{titlepage}
    ...
    {\Large \textbf{LABORATORIUM PRAKTIKUM INFORMATIKA}}\\[0.2cm]
    {\large Fakultas Informatika -- Universitas Telkom}\\[1.5cm]

    \rule{\linewidth}{1.5pt}\\[0.5cm]
    {\Huge \textbf{\color{telkomred}LAPORAN AKHIR TUGAS BESAR}}\\[0.5cm]
    {\Large \textbf{JARINGAN KOMPUTER}}\\[0.8cm]
    ...
    Quality of Service}}\\[2cm]

    \begin{tabular}{ll}
        \textbf{Mata Kuliah}        & : Jaringan Komputer \\[4pt]
        \textbf{Dosen Pengampu}     & : [Nama Dosen Pengampu] \\[4pt]
        \textbf{Program Studi}      & : S1 Informatika \\[4pt]
        \textbf{Fakultas}           & : Informatika \\[4pt]
        \textbf{Universitas}        & : Universitas Telkom \\[4pt]
        \textbf{Tanggal Pengumpulan}& : 14 Juni 2026 \\
    \end{tabular}
    ...
    \vfill
    {\small\color{gray}created by STO -- Laboratorium Praktikum Informatika}
\end{titlepage}
...
\texttt{proxy.py} berperan sebagai \textit{gateway} antara client dan web server. Proxy menggunakan \texttt{socket.SOCK\_STREAM} pada port \textbf{8080}, dengan model konkurensi \textit{thread-per-connection} yang serupa dengan web server.
...
Statistik akhir (Min/Avg/Max RTT, jitter, dan packet loss) dihitung setelah seluruh 10 paket terkirim, sesuai dengan rumus pada Subbagian~\ref{subsec:rumus-qos}.
...
\begin{tikzpicture}[
    ...
    \draw[arr] (w3) -- (shared);
\end{tikzpicture}
\caption{Model konkurensi thread-per-connection pada webserver.py dan proxy.py}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Beban sistem (jumlah client konkuren):} Semakin banyak \textit{thread} yang aktif menangani koneksi pada proxy dan web server, semakin besar kemungkinan terjadi antrian pemrosesan (\textit{scheduling delay}) pada CPU, yang berdampak pada peningkatan RTT rata-rata.
...
\midrule
Connection refused saat client mengakses proxy &
Urutan eksekusi salah; \texttt{proxy.py} dijalankan sebelum \texttt{webserver.py}, atau port 8080/8000 sedang digunakan proses lain &
Memastikan urutan start: \texttt{webserver.py} $\rightarrow$ \texttt{proxy.py} $\rightarrow$ \texttt{client.py}; memeriksa port dengan \texttt{netstat}/\texttt{lsof} sebelum eksekusi \\
\midrule
Timeout pada pengujian UDP (\texttt{run\_udp}) &
Firewall bawaan OS memblokir trafik UDP pada port 9000, atau UDP server belum aktif &
Menonaktifkan firewall sementara pada port 9000, atau menambahkan \textit{inbound rule} khusus UDP; memastikan thread \texttt{start\_udp\_server} berjalan sebelum pengujian \\
\midrule
Cache tidak terbentuk pada direktori \texttt{proxy\_cache/} &
Direktori cache belum memiliki izin tulis, atau path relatif berbeda saat dijalankan dari direktori kerja yang berisi folder \texttt{proxy\_cache/} &
Memastikan \texttt{proxy.py} dijalankan dari direktori kerja yang berisi folder \texttt{proxy\_cache/}; pengecekan \texttt{os.path.exists} dan \texttt{os.makedirs} pada inisialisasi \\
\midrule
Response HTML kosong / parsing gagal &
Format request tidak sesuai \texttt{GET /path HTTP/1.1\textbackslash r\textbackslash n\textbackslash r\textbackslash n} (double CRLF) &
Memvalidasi format request pada \texttt{run\_tcp()} agar selalu menambahkan \texttt{\textbackslash r\textbackslash n\textbackslash r\textbackslash n} di akhir header \\
\midrule
Multi-client tampak \textit{blocking} &
Lupa memanggil \texttt{.start()} pada thread, atau memanggil \texttt{.join()} di dalam loop \texttt{accept()} &
Memastikan setiap koneksi baru langsung dijalankan dengan \texttt{threading.Thread(..., daemon=True).start()} tanpa \texttt{join()} pada loop utama \\
\midrule
Wireshark tidak menangkap paket yang diharapkan &
Antarmuka jaringan yang dipilih salah, atau filter capture tidak sesuai &
Memilih antarmuka aktif sesuai IP perangkat; menggunakan filter \texttt{tcp.port==8000 || tcp.port==8080 || udp.port==9000} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
    ...
    \item \texttt{webserver.py} berhasil menangani request HTTP \texttt{GET} dengan response \texttt{200 OK} dan \texttt{404 Not Found}, serta menjalankan UDP Echo Server pada port 9000 secara paralel menggunakan threading.
    \item \texttt{proxy.py} berhasil mengimplementasikan forwarding, mekanisme caching berbasis hash MD5 (Cache HIT/MISS), serta penanganan galat \texttt{502 Bad Gateway} dan \texttt{504 Gateway Timeout} ketika web server tidak terjangkau.
    \item \texttt{client.py} berhasil mengimplementasikan dua mode operasi: mode TCP untuk request HTTP melalui proxy, dan mode UDP untuk pengujian QoS (RTT, jitter, packet loss) sesuai spesifikasi (10 paket, timeout 1 detik, format payload \texttt{"Ping <seq> <timestamp>"}).
    ...
    \item Model konkurensi \textit{thread-per-connection} terbukti mampu menangani minimal 5 client secara simultan tanpa \textit{crash}, meskipun terdapat potensi \textit{race condition} pada penulisan cache untuk URL yang identik dan diakses bersamaan.
\end{enumerate}
...
\begin{itemize}[noitemsep]
    ...
    \item Penulisan cache belum dilindungi oleh mekanisme \textit{locking}, sehingga berpotensi terjadi \textit{race condition} pada akses konkuren terhadap URL yang sama.
    ...
    \item Penanganan \texttt{500 Internal Server Error} pada \texttt{webserver.py} belum diimplementasikan secara eksplisit (berkas \texttt{status/500.html} tersedia namun belum dipicu oleh \textit{exception handler} tertentu).
\end{itemize}
